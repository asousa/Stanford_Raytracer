function I = adaptGK(fun,a,b,tol,maxLevel)
...
I = doAdaptGK(fun,a,b,tol,maxLevel,0,0);

% =====================
function I = doAdaptGK(fun,a,b,tol,maxLevel,level,I1);
...
if level == 1              %  on first call, I1 has not been evaluated
   [I1,ae1] = gaussKronrod15(fun,a,b);
end
c = a + 0.5*(b-a);
[I2left,ae2left]   = gaussKronrod15(fun,a,c);
[I2right,ae2right] = gaussKronrod15(fun,c,b);
...
if abs(I2-I1) < max(tol(1)*abs(I1),tol(2))   %  Is difference between levels significant?
  ...
  I =   doAdaptGK(fun,a,c,[tol(1) tol(2)/2],maxLevel,level,I2left) ...      %  I_left
      + doAdaptGK(fun,c,b,[tol(1) tol(2)/2],maxLevel,level,I2right);        %  I_right
end
